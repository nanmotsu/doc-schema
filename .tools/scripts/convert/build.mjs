...
// システムにインストール済みの Chrome を優先して使用する。
// Puppeteer 独自の Chrome (要ダウンロード) より先に既存インストールを探す。
...
function buildHeaderFooterOptions(cfg) {
    ...
    const makeTemplate = (section) => {
        ...
        // puppeteer のヘッダー/フッターはデフォルトで font-size が 0 になるため、
        // <style> タグで全要素に明示的なフォントサイズを適用する必要がある。
...
const assetsBaseAbs = resolveAssetsBaseAbs(meta, WORKSPACE_ROOT, srcDir);
